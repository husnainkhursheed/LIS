                        ...
                        <div class="col my-2">
                            <nav class="navbar">
                                <div class="container-fluid p-0">
                                    <form class="d-flex" method="GET" action="{{ route('note.index') }}">
                                    ...
                                    <form class="d-flex" method="GET" action="{{ route('note.index') }}">
                                        ...
                                        <select class="form-select sort-dropdown" aria-label="Default select example"
                                            ...
                                            <option value="note_code"
                                                {{ request('sort_by') == 'note_code' ? 'selected' : '' }}>NoteCode</option>
                                            <option value="department"
                                                {{ request('sort_by') == 'department' ? 'selected' : '' }}>Department
                                            </option>
                                            <option value="comment"
                                                {{ request('sort_by') == 'comment' ? 'selected' : '' }}>Comment
                                            </option>
                                        </select>
                                    </form>
                                </div>
    ...
    <div class="modal fade" id="showModal" tabindex="-1" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content border-0">
                ...
                <form class="tablelist-form" id="leadtype_form" action="{{ url("/note") }}" method="Post" autocomplete="off">
                    @csrf
...
@section('script')
    ...
    <script>
        ...
        $('.edit-item-btn').on('click', function() {
            ...
            $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#leadtype_form').attr('action', '{{ url("/note") }}/' + itemId);
        ...
        function resetModal() {
            ...
            // Change the button text
            $('#add-btn').html("Add");
            $('#leadtype_form').attr('action', '{{ url("/note") }}');
            // if ( $('#patch').length) {
            //     $('#patch').remove();
            // }
            $('#note_code').val('');
